...
export function getActionItemLabel(item: MenuItem): string | { translationKey: string, params?: Record<string, any> } {
  if (item.isAction && item.actionInputValue) {
    return {
      translationKey: 'filter_bar.action.ask_ai',
      params: { input: truncateText(item.actionInputValue, 30) }
    }
  }
  return item.label
}
...
export function buildFilterPlaceholder(
  ...
  options: { maxProperties?: number; hasActions?: boolean; t?: (key: string, params?: Record<string, any>) => string } = {}
): string {
  const { maxProperties = 3, hasActions = false, t } = options

  if (filterProperties.length === 0) {
    if (t) {
      return hasActions 
        ? t('filter_bar.placeholder.add_filters_or_ask_ai')
        : t('filter_bar.placeholder.add_filters')
    }
  ...
  if (t) {
    if (hasActions) {
      return t('filter_bar.placeholder.filter_by_with_ai', { propertyNames, suffix })
    } else {
      return t('filter_bar.placeholder.filter_by', { propertyNames, suffix })
    }
  }
