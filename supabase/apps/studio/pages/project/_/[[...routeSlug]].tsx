...
const GenericProjectPage: NextPage = () => {
  ...
  return (
    <div className="h-screen flex flex-col">
      <Header />
      <PageLayout className="flex-grow min-h-0" title={t('home.select_project')}>
              ...
              <div className="flex items-center gap-x-2">
                <Select_Shadcn_ value={selectedSlug} onValueChange={setSlug}>
                  <SelectTrigger_Shadcn_ size="tiny" className="w-60 truncate">
                    <div className="flex items-center gap-x-2">
                      <p className="text-xs text-foreground-light">{t('home.organization')}:</p>
                      <SelectValue_Shadcn_ placeholder={t('home.select_organization')} />
                    </div>
                  </SelectTrigger_Shadcn_>
          ...
          <ScaffoldSection isFullWidth className="flex-grow pt-0 flex flex-col gap-y-4 h-px">
            {isLoadingOrganizations ? (
              ...
              <Alert_Shadcn_ variant="warning">
                <AlertTriangleIcon />
                <AlertTitle_Shadcn_>{t('home.failed_to_load_orgs')}</AlertTitle_Shadcn_>
                <AlertDescription_Shadcn_>{t('home.try_refreshing')}</AlertDescription_Shadcn_>
              </Alert_Shadcn_>
            ) : organizations.length === 0 ? (
              <NoOrganizationsState />
