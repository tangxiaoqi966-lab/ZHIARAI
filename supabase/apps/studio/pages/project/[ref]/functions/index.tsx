                  ...
                  <div className="space-y-4">
                    ...
                    <Card>
                      <Table>
                        <TableHeader>
                          <TableRow>
                            <TableHead>{t('functions.table_name')}</TableHead>
                            <TableHead>{t('functions.table_url')}</TableHead>
                            <TableHead className="hidden 2xl:table-cell">{t('functions.table_created')}</TableHead>
                            <TableHead className="lg:table-cell">{t('functions.table_updated')}</TableHead>
                            <TableHead className="lg:table-cell">{t('functions.table_deployments')}</TableHead>
                          </TableRow>
                        </TableHeader>
                        ...
                        <TableBody>
                          <>
                            {filteredFunctions.length > 0 ? (
                              ...
                              <TableRow>
                                <TableCell colSpan={5}>
                                  <p className="text-sm text-foreground">{t('common.no_results_found')}</p>
                                  <p className="text-sm text-foreground-light">
                                    {t('common.search_no_results', { search })}
                                  </p>
                                </TableCell>
                              </TableRow>
                            )}
...
const EdgeFunctionsPageLayout = ({ children }: { children: React.ReactNode }) => {
  const { t } = useTranslation()
      ...
      <EdgeFunctionsLayout>
        <div className="w-full min-h-full flex flex-col items-stretch">
          <PageHeader size="large">
            <PageHeaderMeta>
              <PageHeaderSummary>
                <PageHeaderTitle>{t('functions.page_title')}</PageHeaderTitle>
                <PageHeaderDescription>
                  {t('functions.page_description')}
                </PageHeaderDescription>
              </PageHeaderSummary>
              <PageHeaderAside>
...
EdgeFunctionsPage.getLayout = (page: React.ReactElement) => {
  return <EdgeFunctionsPageLayout>{page}</EdgeFunctionsPageLayout>
}
