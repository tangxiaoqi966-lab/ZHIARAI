      ...
      <div className="flex items-center gap-x-2">
        ...
        {!snap.allRowsSelected ? (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="start" className="w-40">
              <DropdownMenuItem onClick={() => onCopyRows('csv')}>{t('header.copy_as_csv')}</DropdownMenuItem>
              <DropdownMenuItem onClick={() => onCopyRows('sql')}>{t('header.copy_as_sql')}</DropdownMenuItem>
              <DropdownMenuItem onClick={() => onCopyRows('json')}>{t('header.copy_as_json')}</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        ) : (
          <ButtonTooltip
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className={snap.allRowsSelected ? 'w-52' : 'w-40'}>
            <DropdownMenuItem onClick={onRowsExportCSV}>{t('header.export_as_csv')}</DropdownMenuItem>
            <DropdownMenuItem onClick={onRowsExportSQL}>{t('header.export_as_sql')}</DropdownMenuItem>
            {snap.allRowsSelected ? (
              <DropdownMenuItem className="group" onClick={() => setShowExportModal(true)}>
                <div>
                  <p className="group-hover:text-foreground">{t('header.export_via_cli')}</p>
                  <p className="text-foreground-lighter">Recommended for large tables</p>
                </div>
              </DropdownMenuItem>
            ) : (
              <DropdownMenuItem onClick={onRowsExportJSON}>{t('header.export_as_json')}</DropdownMenuItem>
            )}
          </DropdownMenuContent>
        </DropdownMenu>
