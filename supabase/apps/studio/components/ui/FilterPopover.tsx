      ...
      >
        <div className="border-b border-overlay bg-surface-200 rounded-t pb-1 px-3">
          <span className="text-xs text-foreground-light">
            {title ?? t('home.filter.select', { name: name.toLowerCase() })}
          </span>
        </div>
        {search !== undefined && (
          <Input
            ...
            placeholder={t('home.filter.search_placeholder')}
            actions={
              (search ?? '').length > 0 ? (
        ...
        {(search ?? '').length > 0 && options.length === 0 && (
          <p className="text-xs text-foreground-lighter pt-3 px-3">{t('home.filter.no_results')}</p>
        )}
        ...
        <div className="flex items-center justify-end gap-2 border-t border-overlay bg-surface-200 py-2 px-3">
          ...
          >
            {clearButtonText === 'Clear' ? t('home.filter.clear') : clearButtonText}
          </Button>
          <Button
            type="primary"
            onClick={() => {
