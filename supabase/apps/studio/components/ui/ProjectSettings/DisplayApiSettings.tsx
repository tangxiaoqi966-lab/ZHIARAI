      ...
      title={
        showTitle && (
          <div className="space-y-3">
            <h5 className="text-base">{t('api_settings.project_api_keys')}</h5>
            <p className="text-sm text-foreground-light">
              {t('api_settings.project_api_description')}
            </p>
          </div>
        )
      }
    >
      {isLoading ? (
        <div className="flex items-center justify-center py-8 space-x-2">
          ...
          <p className="text-sm text-foreground-light">{t('api_settings.retrieving_api_keys')}</p>
        </div>
      ) : !canReadAPIKeys ? (
        <div className="flex items-center py-8 px-8 space-x-2">
          <AlertCircle size={16} strokeWidth={1.5} />
          <p className="text-sm text-foreground-light">
            {t('api_settings.no_permission_to_view')}
          </p>
        </div>
        ...
        <div className="flex items-center justify-center py-8 space-x-2">
          <AlertCircle size={16} strokeWidth={1.5} />
          <p className="text-sm text-foreground-light">
            {isProjectSettingsError ? t('api_settings.failed_retrieve_api_keys') : t('api_settings.failed_update_jwt_secret')}
          </p>
        </div>
        ...
        <div className="flex items-center justify-center py-8 space-x-2">
          ...
          <p className="text-sm text-foreground-light">
            {isProjectSettingsLoading || isApiKeysEmpty
              ? t('api_settings.retrieving_api_keys')
              : t('api_settings.jwt_secret_updating')}
          </p>
        </div>
      ) : (
          ...
          >
            <FormLayout
              ...
              description={
                x.tags === 'service_role' ? (
                  <>
                    {t('api_settings.service_role_description')}{' '}
                    {showLegacyText && (
                      <span>
                        Prefer using{' '}
                        <Link
                        ...
                        >
                          {t('api_settings.secret_api_keys')}
                        </Link>{' '}
                        instead.
                      </span>
                    )}
                  ...
                  <>
                    {t('api_settings.anon_key_description')}{' '}
                    {showLegacyText && (
                      <span>
                        Prefer using{' '}
                        <Link
                          ...
                          className="text-link underline"
                        >
                          {t('api_settings.publishable_api_keys')}
                        </Link>{' '}
                        instead.
                      </span>
                    )}
