...
export const AIAssistant = ({ className }: AIAssistantProps) => {
  ...
  const placeholderText = hasMessages
    ? t('ai_assistant.ask_follow_up_question')
    : (snap.sqlSnippets ?? [])?.length > 0
      ? t('ai_assistant.ask_question_or_make_change')
      : t('ai_assistant.chat_to_postgres_placeholder')
      ...
      <div className={cn('flex flex-col h-full w-full md:h-full max-h-[100dvh]', className)}>
        ...
        <div className="px-3 pb-3 z-20 relative">
          ...
          {isSuccess && !isApiKeySet && (
            <Admonition
              type="default"
              title={t('ai_assistant.openai_api_key_not_set')}
              description={
                  <Markdown
                    content={t('ai_assistant.openai_api_key_description')}
                  />
                }
            />
          )}
