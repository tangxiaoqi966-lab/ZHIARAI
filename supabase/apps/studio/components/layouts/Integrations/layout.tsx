...
const IntegrationsLayout = ({ children }: PropsWithChildren) => {
  ...
  return (
    <ProjectLayout
      ...
      product={t('integrations.product')}
      isBlocking={false}
      productMenu={
        <>
          <ProductMenu
            ...
            menu={generateIntegrationsMenu({ projectRef: project?.ref, flags: { showWrappers }, t })}
          />
          <Separator />
          <div className="px-4 py-6 md:px-6">
            <Menu.Group
              title={
                <div className="flex flex-col space-y-2 uppercase font-mono">
                  <span>{t('integrations.categories.installed')}</span>
                </div>
              }
            />
            ...
            {isError && (
              <AlertError
                showIcon={false}
                error={error}
                subject={t('integrations.errors.failed_to_retrieve_installed')}
              />
            )}
            {isSuccess && (
              <div>
...
const generateIntegrationsMenu = ({
  ...
  return [
    {
      title: t('integrations.categories.explore'),
      items: [
        {
          name: t('integrations.categories.all'),
          key: 'integrations',
        ...
        ...(showWrappers
          ? [
              {
                name: t('integrations.categories.wrapper'),
          ...
          : []),
        {
          name: t('integrations.categories.postgres_extension'),
