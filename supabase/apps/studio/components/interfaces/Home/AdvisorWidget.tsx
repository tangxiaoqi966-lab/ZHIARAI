...
export const AdvisorWidget = () => {
  ...
  if (totalIssues === 0) {
    titleContent = <h2>{t('advisor.no_issues_available')}</h2>
  } else {
    const issuesText = totalIssues === 1 ? t('advisor.issue') : t('advisor.issues')
    ...
    titleContent = (
      <h2>
        {numberDisplay} {issuesText} {t('advisor.need')}
        {totalIssues === 1 ? '' : ''} <span className={attentionClassName}>{t('advisor.attention')}</span>
      </h2>
    )
  }
      ...
      <div className="h-full">
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between w-full group">
                    ...
                    >
                      <AiAssistantDropdown
                        label={t('advisor.ask_assistant')}
                        iconOnly
                        tooltip={t('advisor.help_fix_issue')}
        ...
        {!isLoading && errorCount === 0 && warningCount === 0 && (
          <div className="flex-1 flex flex-col h-full items-center justify-center gap-2">
            ...
            <p className="text-sm text-foreground-light">{title === 'Security' ? t('advisor.no_security_issues_found') : t('advisor.no_performance_issues_found')}</p>
          </div>
        )}
      </div>
    )
      ...
      <div className="grid grid-cols-1 @xl:grid-cols-2 gap-4">
            ...
            <CardHeader className="h-10 py-0 pl-4 pr-2 flex flex-row items-center justify-between flex-0">
              ...
              <ButtonTooltip
                ...
                tooltip={{
                  content: {
                    side: 'bottom',
                    text: selectedTab === 'security' ? t('advisor.open_security_advisor') : t('advisor.open_performance_advisor'),
        ...
        <Card className="h-80 flex flex-col">
          <CardHeader className="h-10 flex-row items-center justify-between py-0 pl-4 pr-2">
            <CardTitle>{t('advisor.slow_queries')}</CardTitle>
            <ButtonTooltip
              asChild
              type="text"
              className="!mt-0 w-7"
              ...
              tooltip={{
                content: {
                  side: 'bottom',
                  text: t('advisor.open_query_performance_advisor'),
                },
              }}
            >
            ...
            {isLoadingSlowestQueries ? (
              ...
              <div className="flex-1 flex flex-col h-full items-center justify-center gap-2">
                ...
                <p className="text-sm text-foreground-light">
                  {t('advisor.no_slow_queries_found')}
                </p>
              </div>
            ) : (
