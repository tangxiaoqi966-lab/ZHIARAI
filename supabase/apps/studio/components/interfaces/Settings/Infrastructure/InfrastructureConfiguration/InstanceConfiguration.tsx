  ...
  return (
    <div className={cn('nowheel', diagramOnly ? 'h-full' : 'border-y')}>
      ...
      >
        ...
        {isError && <AlertError error={error} subject={t('settings.infrastructure.failed_retrieve_replicas')} />}
                    ...
                    tooltip={{
                      content: {
                        ...
                        text: !canManageReplicas
                          ? t('settings.infrastructure.deploy_replica_permission_error')
                          : isOrioleDb
                            ? t('settings.infrastructure.read_replicas_not_supported')
                            : undefined,
                      },
                    }}
                  >
                    {t('settings.infrastructure.deploy_new_replica')}
                  </ButtonTooltip>
                  {replicas.length > 0 && (
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-52 *:space-x-2">
                        <DropdownMenuItem asChild>
                          <Link href={`/project/${projectRef}/settings/compute-and-disk`}>
                            {t('settings.infrastructure.resize_databases')}
                          </Link>
                        </DropdownMenuItem>
                        <DropdownMenuSeparator />
                        <DropdownMenuItem onClick={() => setShowDeleteAllModal(true)}>
                          <div>{t('settings.infrastructure.remove_all_replicas')}</div>
                        </DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  )}
