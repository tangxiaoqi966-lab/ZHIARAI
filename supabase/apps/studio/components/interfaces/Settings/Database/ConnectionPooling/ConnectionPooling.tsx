...
export const ConnectionPooling = () => {
  ...
  const onSubmit: SubmitHandler<z.infer<typeof PoolingConfigurationFormSchema>> = async (data) => {
    ...
    updatePoolerConfig(
      ...
      {
        onSuccess: (data) => {
          toast.success(t('connection_pooling.successfully_updated'))
          if (data) {
            form.reset({
  ...
  return (
    <PageSection id="connection-pooler">
      <PageSectionMeta>
        <PageSectionSummary>
          <PageSectionTitle>{t('connection_pooling.title')}</PageSectionTitle>
        </PageSectionSummary>
        <PageSectionAside>
          <DocsButton
      ...
      <PageSectionContent className="space-y-4">
        {isSuccessAddons && !disablePoolModeSelection && !hasIpv4Addon && (
          <Admonition
            type="default"
            title={t('connection_pooling.dedicated_pooler_ipv4_incompatible_title')}
          >
            <p className="!m-0">
              <Trans
                i18nKey="connection_pooling.dedicated_pooler_ipv4_incompatible_desc"
                components={{
                  link: <InlineLink href={`/project/${projectRef}/settings/addons?panel=ipv4`} />,
                }}
              />
            </p>
          </Admonition>
            ...
            <FormActions
              ...
              helper={
                !canUpdateConnectionPoolingConfiguration
                  ? t('connection_pooling.update_permission_error')
                  : undefined
              }
            />
          }
        >
          <Panel.Content>
            ...
            {isErrorPgbouncerConfig && (
              <AlertError
                error={pgbouncerConfigError}
                subject={t('connection_pooling.failed_retrieve_config')}
              />
            )}
            {connectionPoolingUnavailable && (
              <Admonition
                type="default"
                title={t('connection_pooling.unable_retrieve_config')}
                description={t('connection_pooling.start_new_project')}
              />
            )}
              ...
              <>
                <div className="flex flex-row gap-2 justify-between w-full">
                  <div className="flex flex-col text-sm">
                    <h5 className="text-foreground font-normal">
                      {t('connection_pooling.connection_poolers')}
                    </h5>
                    <p className="text-foreground-lighter">
                      {t('connection_pooling.config_shared')}
                    </p>
                  </div>
                  <div className="flex flex-row gap-1 items-center">
                    <Badge>{t('connection_pooling.shared')}</Badge>
                    {!disablePoolModeSelection && <Badge>{t('connection_pooling.dedicated')}</Badge>}
                  </div>
                </div>
                ...
                <Form_Shadcn_ {...form}>
                  ...
                  >
                    <FormField_Shadcn_
                          ...
                          label={t('connection_pooling.connection_pool_size')}
                          description={
                            <p>
                              {t('connection_pooling.connection_pool_size_desc', {
                            ...
                            (default_pool_size ?? 15) > maxConnData.maxConnections * 0.8 && (
                              <Alert_Shadcn_ variant="warning" className="mt-2">
                                <AlertTitle_Shadcn_ className="text-foreground">
                                  {t('connection_pooling.pool_size_warning_title', {
                                    max: maxConnData.maxConnections,
                                  })}
                                </AlertTitle_Shadcn_>
                                <AlertDescription_Shadcn_>
                                  {t('connection_pooling.pool_size_warning_desc')}
                                </AlertDescription_Shadcn_>
                              </Alert_Shadcn_>
                            )}
                        </FormItemLayout>
                    ...
                    <FormField_Shadcn_
                      ...
                      render={({ field }: { field: any }) => (
                        <FormItemLayout
                          layout="flex-row-reverse"
                          label={t('connection_pooling.max_client_connections')}
                          className="[&>div]:md:w-1/2 [&>div]:xl:w-2/5 [&>div>div]:w-full [&>div>div>div]:min-w-100"
                          description={
                            <>
                              <p>
                                {t('connection_pooling.max_client_connections_desc', {
                                  count: defaultMaxClientConn,
                                  computeSize: computeSize,
                                })}{' '}
                                <InlineLink
                                  href={`${DOCS_URL}/guides/database/connection-management#configuring-supavisors-pool-size`}
                                >
                                  {t('connection_pooling.learn_more')}
                                </InlineLink>
                              </p>
                            </>
                          }
