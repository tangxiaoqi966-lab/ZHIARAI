        ...
        <Button className="rounded-r-none" type="default" size="tiny" iconRight={<ChevronDown />}>
          ...
          >
            {config.channelName.length > 0
              ? `${t('realtime.channel')}: ${config.channelName}`
              : t('realtime.join_channel')}
          </p>
        </Button>
      </PopoverTrigger_Shadcn_>
                          ...
                          <div className="flex flex-row">
                            <FormControl_Shadcn_>
                              <Input_Shadcn_
                                ...
                                placeholder={t('realtime.enter_channel_name')}
                              />
                            </FormControl_Shadcn_>
                      ...
                      <FormItem_Shadcn_ className="">
                        <div className="flex flex-row items-center gap-x-2">
                          ...
                          <FormLabel_Shadcn_ className="text-xs">
                            {t('realtime.is_channel_private')}
                          </FormLabel_Shadcn_>
                        </div>
                        <FormDescription_Shadcn_ className="text-xs text-foreground-lighter mt-2">
                          {t('realtime.is_channel_private_description')}
                        </FormDescription_Shadcn_>
                      </FormItem_Shadcn_>
                    )}
                  />
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-x-2">
                <p className="text-foreground text-xs">
                  {t('realtime.currently_joined')}{' '}
                  <span className={config.isChannelPrivate ? 'text-brand' : 'text-warning'}>
                    {config.isChannelPrivate ? t('realtime.private') : t('realtime.public')}
                  </span>{' '}
                  {t('realtime.channel').toLowerCase()}:
                </p>
              ...
              <p className="text-xs text-foreground-lighter mt-2">
                {t('realtime.leave_channel_warning')}
              </p>
              <Button
                type="default"
                onClick={() => onChangeConfig({ ...config, channelName: '', enabled: false })}
              >
                {t('realtime.leave_channel')}
              </Button>
            </div>
          )}
        </div>
