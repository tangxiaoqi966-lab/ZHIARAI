    ...
    <>
      <Popover_Shadcn_ open={open} onOpenChange={onOpen}>
        <PopoverTrigger_Shadcn_ asChild>
          ...
          >
            {isFiltered ? (
              <>
                <span className="mr-1">{t('realtime.filtered_by')}</span>
                <Badge variant="success">table: {config.table}</Badge>
              </>
            ) : (
              <span className="mr-1">{t('realtime.filter_messages')}</span>
            )}
          </Button>
        </PopoverTrigger_Shadcn_>
        <PopoverContent_Shadcn_ className="p-0 w-[365px]" align="start">
          <div className="border-b border-overlay text-xs px-4 py-3 text-foreground">
            {t('realtime.listen_to_event_types')}
          </div>
          <div className="py-3 px-4 border-b border-overlay">
            ...
            <p className="text-xs text-foreground-light pt-1">
              {t('realtime.presence_description')}
            </p>
          </div>
          <div className="py-3 px-4 border-b border-overlay">
            ...
            <p className="text-xs  text-foreground-light pt-1">
              {t('realtime.broadcast_description')}
            </p>
          </div>
          <div className="py-3 px-4 border-b border-overlay">
            <div className="flex items-center justify-between">
            ...
            <p className="text-xs text-foreground-light pt-1">
              {t('realtime.database_changes_description')}
            </p>
            {!config.enableDbChanges && (
              <p className="text-xs text-foreground-light mt-2">
                <Trans
                  i18nKey="realtime.enable_realtime_publications"
                  components={{
                    link: (
                      <InlineLink
          ...
          {tempConfig.enableDbChanges && config.enableDbChanges && (
            <>
              <div className="border-b border-overlay text-xs px-4 py-3 text-foreground">
                {t('realtime.filter_messages_from_database_changes')}
              </div>
              ...
              <div className="border-b border-overlay p-4 flex flex-col gap-2">
                ...
                <p className="text-xs text-foreground-light pl-[80px]">
                  <Trans
                    i18nKey="realtime.learn_more_filtering"
                    components={{
                      link: (
                        <Link
                          className="underline"
          ...
          <div className="px-4 py-2 gap-2 flex justify-end">
            ...
            <Button onClick={() => setApplyConfigOpen(true)}>{t('realtime.apply')}</Button>
          </div>
        </PopoverContent_Shadcn_>
      </Popover_Shadcn_>
      <ConfirmationModal
        title={t('realtime.confirm_clear_messages_title')}
        variant="destructive"
        confirmLabel={t('realtime.confirm')}
        size="small"
      ...
      >
        <p className="text-sm text-foreground-light">
          {t('realtime.confirm_clear_messages_description')}
        </p>
      </ConfirmationModal>
    </>
  )
