...
export function useAttachmentUpload() {
  ...
  const handleFileUpload = useCallback(
    async (event: ChangeEvent<HTMLInputElement>) => {
      ...
      if (items.length + uploadedFiles.length > MAX_ATTACHMENTS) {
        toast(t('support.attachment_too_many', { max: MAX_ATTACHMENTS }))
      }
      event.target.value = ''
    },
    [uploadedFiles]
  ...
  const createAttachments = useCallback(async () => {
    if (!profile?.id) {
      ...
      toast.error(t('support.attachment_upload_failed'))
      return []
    }
...
export function AttachmentUploadDisplay({
  ...
  if (!profile) {
    return (
      <div>
        <h3 className="text-sm text-foreground">{t('support.attachments')}</h3>
        <p className="text-sm text-foreground-lighter mt-2">
          {t('support.attachments_login_required')}
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-y-4">
      <div className="flex flex-col gap-y-1">
        <p className="text-sm text-foreground">{t('support.attachments')}</p>
        <p className="text-sm text-foreground-lighter">
          <Trans
            i18nKey="support.attachments_description"
