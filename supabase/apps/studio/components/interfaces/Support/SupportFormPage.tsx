  ...
  return (
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-y-2">
      <div className="flex items-center space-x-3">
        ...
        <h3 className="m-0 text-lg">{t('support.title')}</h3>
      </div>
          ...
          <TooltipTrigger asChild>
            ...
            >
              <Link href="https://status.supabase.com/" target="_blank" rel="noreferrer">
                {isLoading
                  ? t('support.checking_status')
                  : isError
                    ? t('support.failed_to_check_status')
                    : isIncident
                      ? t('support.active_incident')
                      : isMaintenance
                        ? t('support.scheduled_maintenance')
                        : t('support.all_systems_operational')}
              </Link>
            </Button>
          </TooltipTrigger>
          <TooltipContent side="bottom" align="center">
            {t('support.check_status_page')}
          </TooltipContent>
        </Tooltip>
      </div>
    </div>
...
function SupportFormDirectEmailInfo({ projectRef }: SupportFormDirectEmailInfoProps) {
  ...
  return (
    <Admonition
      type="default"
      title={t('support.having_trouble')}
      description={
        <>
          <p className="!mb-2.5">
            {t('support.email_directly')}
          </p>
          <p className="flex items-center gap-x-1.5 flex-wrap">
            ...
            <span className="inline-flex items-center gap-x-1">
              ...
              <CopyButton
                ...
                iconOnly
                onClick={() => toast.success(t('support.copied_email'))}
              />
            </span>
          </p>
          {hasProjectRef && (
