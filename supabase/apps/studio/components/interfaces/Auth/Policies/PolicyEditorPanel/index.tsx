...
export const PolicyEditorPanel = memo(function ({
  ...
  const onToggleTools = useCallback(() => setShowTools((prev) => !prev), [])
  ...
  const { mutate: executeMutation, isPending: isExecuting } = useExecuteSqlMutation({
    onSuccess: async () => {
      ...
      toast.success(t('auth_policies.successfully_created_policy'))
      onSelectCancel()
    },
  ...
  const { mutate: updatePolicy, isPending: isUpdating } = useDatabasePolicyUpdateMutation({
    onSuccess: () => {
      toast.success(t('auth_policies.successfully_updated_policy'))
      onSelectCancel()
    },
  })
  ...
  const onSubmit = (data: z.infer<typeof FormSchema>) => {
    ...
    if (command === 'insert' && (check === undefined || check.length === 0)) {
      return setFieldError(t('auth_policies.please_provide_sql_check'))
    } else if (command !== 'insert' && (using === undefined || using.length === 0)) {
      return setFieldError(t('auth_policies.please_provide_sql_using'))
    } else {
                    ...
                    {supportWithCheck && (
                      <div className="px-5 py-3 flex items-center gap-x-2">
                        ...
                        <Label_Shadcn_ className="text-xs cursor-pointer" htmlFor="use-check">
                          {t('auth_policies.use_check_expression')}
                        </Label_Shadcn_>
                      </div>
                    )}
                  </div>
                        ...
                        tooltip={{
                          content: {
                            ...
                            text: !canUpdatePolicies
                              ? t('auth_policies.create_policy_permission_error')
                              : undefined,
                          },
                        }}
                      >
                        {t('auth_policies.save_policy')}
                      </ButtonTooltip>
                    </SheetFooter>
                  </div>
                </div>
...
const CloseConfirmationModal = ({ visible, onClose, onCancel }: ConfirmOnCloseModalProps) => {
  ...
  return (
    <ConfirmationModal
      visible={visible}
      title={t('auth_policies.discard_changes')}
      confirmLabel={t('auth_policies.discard')}
      onCancel={onCancel}
      onConfirm={onClose}
    >
      <p className="text-sm text-foreground-light">
        {t('auth_policies.discard_changes_description')}
      </p>
    </ConfirmationModal>
  )
}
