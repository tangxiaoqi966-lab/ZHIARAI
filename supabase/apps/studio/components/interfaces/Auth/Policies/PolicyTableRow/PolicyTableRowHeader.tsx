        ...
        >
          ...
          {!table.rls_enabled && (
            <Badge variant="warning" className="shrink-0">
              {t('auth_policies.rls_disabled')}
            </Badge>
          )}
          {!hasApiAccess && (
            <Badge variant="default" className="shrink-0">
              {t('auth_policies.api_disabled')}
            </Badge>
          )}
        </EditorTablePageLink>
        {isTableLocked && (
          <Badge>
            <span className="flex gap-2 items-center text-xs uppercase text-foreground-lighter">
              <Lock size={12} /> {t('auth_policies.locked')}
            </span>
          </Badge>
        )}
      </div>
          ...
          <div className="flex flex-row justify-end gap-x-2">
              ...
              <ButtonTooltip
                ...
                tooltip={{
                  content: {
                    side: 'bottom',
                    text: !canToggleRLS
                      ? t('auth_policies.toggle_rls_permission_error')
                      : undefined,
                  },
                }}
              >
                {table.rls_enabled ? t('auth_policies.disable_rls') : t('auth_policies.enable_rls')}
              </ButtonTooltip>
            )}
            <ButtonTooltip
              type="default"
            ...
            <ButtonTooltip
              ...
              onClick={() => {
                ...
                aiSnap.newChat({
                  name: t('auth_policies.create_policy_chat_title'),
                  initialInput: t('auth_policies.create_policy_initial_input', { schema: table.schema, table: table.name }),
                })
              }}
              tooltip={{
                content: {
                  ...
                  text:
                    !canToggleRLS || !canCreatePolicies
                      ? t('auth_policies.create_policy_permission_error')
                      : t('auth_policies.create_with_assistant_tooltip'),
                },
              }}
            >
