...
const UtilityPanel = ({
  ...
  const { mutate: upsertContent } = useContentUpsertMutation({
    ...
    onError: async (err, newContent, context) => {
      toast.error(t('sql_editor.chart_update_failed'))
    },
  })
      ...
      <TabsList_Shadcn_ className="flex justify-between gap-2 px-4 overflow-x-auto min-h-[42px]">
        <div className="flex items-center gap-4">
          <TabsTrigger_Shadcn_ className="py-3 text-xs" value="results">
            <span className="translate-y-[1px]">{t('sql_editor.utility.tabs.results')}</span>
          </TabsTrigger_Shadcn_>
          {/* Only show Explain tab if ShowPrettyExplain flag is on */}
          {showPrettyExplain && (
            <TabsTrigger_Shadcn_ className="py-3 text-xs" value="explain">
              <span className="translate-y-[1px]">{t('sql_editor.utility.tabs.explain')}</span>
            </TabsTrigger_Shadcn_>
          )}
          <TabsTrigger_Shadcn_ className="py-3 text-xs" value="chart">
            <span className="translate-y-[1px]">{t('sql_editor.utility.tabs.chart')}</span>
          </TabsTrigger_Shadcn_>
          {result?.rows && (
