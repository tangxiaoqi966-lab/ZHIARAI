    ...
    if (isExecuting) {
      return (
        <div className="flex items-center gap-x-4 px-6 py-4 bg-table-header-light [[data-theme*=dark]_&]:bg-table-header-dark">
          ...
          <p className="m-0 border-0 font-mono text-sm">{t('sql_editor.running')}</p>
        </div>
      )
              ...
              {formattedError.length > 0 && (
                <Tooltip>
                  ...
                  <TooltipContent side="bottom" align="center">
                    <span>{t('sql_editor.copy_error')}</span>
                  </TooltipContent>
                </Tooltip>
              )}
              {!hasHipaaAddon && (
                <AiAssistantDropdown
                  label={t('sql_editor.debug_with_assistant')}
      ...
      return (
        <div className="bg-table-header-light [[data-theme*=dark]_&]:bg-table-header-dark overflow-y-auto">
          <p className="m-0 border-0 px-4 py-4 text-sm text-foreground-light">
            <Trans
              i18nKey="sql_editor.click_run_to_execute"
