...
export const SQLEditor = () => {
    ...
    async (force: boolean = false) => {
      ...
      if (editorRef.current !== null && !isExecuting && project !== undefined) {
        ...
        if (!isValidConnString(connectionString)) {
          return toast.error(t('sql_editor.connection_string_missing'))
        }
    ...
    if (editorRef.current !== null && !isExplainExecuting && project !== undefined) {
      ...
      if (statements.length > 1) {
        snapV2.addExplainResultError(id, {
          message: t('sql_editor.explain_single_statement_only'),
        })
        setActiveUtilityTab('explain')
        return
      }
  ...
  const handleNewQuery = useCallback(
    async (sql: string, name: string) => {
      ...
      try {
        ...
        toast.error(t('sql_editor.create_failed', { error: error.message }))
      }
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const buildDebugPrompt = useCallback(() => {
    ...
    const prompt = t('sql_editor.debug_prompt', { error: errorMessage })
  ...
  const onDebug = useCallback(async () => {
    try {
      ...
      aiSnap.newChat({
        name: t('sql_editor.debug_title'),
        sqlSnippets: [
          (snippet.snippet.content?.sql ?? '').replace(sqlAiDisclaimerComment, '').trim(),
        ],
        initialInput: t('sql_editor.debug_prompt', { error: result.error.message }),
      })
    } catch (error: unknown) {
      ...
      if (isError(error)) {
        toast.error(t('sql_editor.debug_failed'))
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [id, snapV2.results, snapV2.snippets])
    ...
    async (
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorText || t('sql_editor.generate_completion_failed'))
        }
        ...
        toast.error(t('sql_editor.generate_sql_failed', { error: error?.message ?? 'Unknown error' }))
        setIsCompletionLoading(false)
        throw error
      }
    },
                    ...
                    <MonacoEditor
                      ...
                      placeholder={
                        !promptState.isOpen && !editorRef.current?.getValue()
                          ? t('sql_editor.placeholder', { shortcut: os === 'macos' ? 'CMD+K' : 'CTRL+K' })
                          : ''
                      }
                      id={id}
                      snippetName={
                ...
                <Tooltip>
                  <TooltipTrigger>
                    <p className="text-xs">
                      <span className="text-foreground">
                        {t(results.rows.length > 1 ? 'sql_editor.rows_count_plural' : 'sql_editor.rows_count', { count: results.rows.length })}
                      </span>
                      <span className="text-foreground-lighter ml-1">
                        {results.autoLimit !== undefined &&
                          t('sql_editor.limited_rows', { limit: results.autoLimit })}
                      </span>
                    </p>
                  </TooltipTrigger>
                  <TooltipContent className="max-w-xs">
                    <p className="flex flex-col gap-y-1">
                      <span>
                        {t('sql_editor.limit_results_description')}
                      </span>

                      <span className="text-foreground-light">
                        {t('sql_editor.limit_results_change')}
                      </span>
                    </p>
                  </TooltipContent>
                </Tooltip>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent className="w-40" align="end">
                      <DropdownMenuRadioGroup
                        ...
                        onValueChange={(val: any) => snapV2.setLimit(Number(val))}
                      >
