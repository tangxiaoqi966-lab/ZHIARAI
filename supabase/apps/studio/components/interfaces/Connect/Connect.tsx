...
export const Connect = () => {
  ...
  const tabLabels = useMemo(() => ({
    'connection-string': t('connect.tabs.connection_string'),
    'frameworks': t('connect.tabs.app_frameworks'),
    'mobiles': t('connect.tabs.mobile_frameworks'),
    'orms': t('connect.tabs.orms'),
    'api-keys': t('connect.tabs.api_keys'),
    'mcp': t('connect.tabs.mcp'),
  }), [t])
            ...
            <TabsList_Shadcn_ className={cn('flex overflow-x-scroll gap-x-4', DIALOG_PADDING_X)}>
              {connectionTypes.map((type) => (
                <TabsTrigger_Shadcn_ key={type.key} value={type.key} className="px-0">
                  {tabLabels[type.key as keyof typeof tabLabels]}
                </TabsTrigger_Shadcn_>
              ))}
            </TabsList_Shadcn_>
          ) : (
