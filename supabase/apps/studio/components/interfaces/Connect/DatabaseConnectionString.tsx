...
export const DatabaseConnectionString = () => {
  ...
  const translatedMethodOptions = useMemo(() => ({
    direct: {
      label: t('connect.methods.direct'),
      description: t('connect.methods.direct_description'),
    },
    transaction: {
      label: t('connect.methods.transaction'),
      description: t('connect.methods.transaction_description'),
    },
    session: {
      label: t('connect.methods.session'),
      description: t('connect.methods.session_description'),
    },
  }), [t])
  const translatedConnectionParameters = useMemo(() => ({
    host: {
      ...CONNECTION_PARAMETERS.host,
      description: t('connect.connection_parameters.host'),
    },
    port: {
      ...CONNECTION_PARAMETERS.port,
      description: t('connect.connection_parameters.port'),
    },
    database: {
      ...CONNECTION_PARAMETERS.database,
      description: t('connect.connection_parameters.database'),
    },
    user: {
      ...CONNECTION_PARAMETERS.user,
      description: t('connect.connection_parameters.user'),
    },
    pool_mode: {
      ...CONNECTION_PARAMETERS.pool_mode,
      description: t('connect.connection_parameters.pool_mode'),
    },
  }), [t])
    ...
    <div className="flex flex-col">
      <div className={cn('w-full flex flex-col items-start gap-2 lg:gap-3', DIALOG_PADDING_X)}>
              ...
              <SelectContent_Shadcn_>
                {DATABASE_CONNECTION_TYPES.map((type) => (
                  <SelectItem_Shadcn_ key={type.id} value={type.id}>
                    {t(`connect.database_types.${type.id}`)}
                  </SelectItem_Shadcn_>
                ))}
              </SelectContent_Shadcn_>
            </Select_Shadcn_>
        ...
        <p className="text-xs inline-flex items-center gap-1 text-foreground-lighter">
          <BookOpen size={12} strokeWidth={1.5} className="-mb-px" /> {t('connect.learn')}
          <InlineLink
            title={t('connect.read_docs')}
            className="flex items-center gap-x-1"
            href={`${DOCS_URL}/guides/database/connecting-to-postgres`}
          >
            {t('connect.read_docs')} <ExternalLink size={12} strokeWidth={1.5} />
          </InlineLink>
        </p>
      </div>
            ...
            <div className="flex flex-col w-full">
              <div className="grid lg:grid-cols-3 gap-4 lg:gap-5 py-8 px-4 md:px-7">
                <StepLabel number={++stepNumber} className="items-start">
                  {t('connect.steps.install_following')}
                </StepLabel>
              ...
              {exampleFiles && exampleFiles?.length > 0 && (
                <div className="grid lg:grid-cols-3 gap-4 lg:gap-5 border-t py-8 px-4 md:px-7">
                  <StepLabel number={++stepNumber} className="items-start">
                    {t('connect.steps.add_file_to_project')}
                  </StepLabel>
          ...
          <div>
            {hasCodeExamples && (
              <div className="px-4 md:px-7 pt-8">
                <StepLabel number={++stepNumber}>{t('connect.steps.connect_to_database')}</StepLabel>
              </div>
            )}
            <div className="px-4 md:px-7 py-8">
              {selectedMethod === 'direct' && (
                <ConnectionPanel
                  type="direct"
                  title={translatedMethodOptions.direct.label}
                  contentType={contentType}
                  lang={lang}
                  fileTitle={fileTitle}
                  description={translatedMethodOptions.direct.description}
                  connectionString={connectionStrings['direct'][selectedTab]}
                  ipv4Status={{
                    type: !ipv4Addon ? 'error' : 'success',
                    title: !ipv4Addon ? t('connect.not_ipv4_compatible') : t('connect.ipv4_compatible'),
                    description:
                      !sharedPoolerPreferred && !ipv4Addon
                        ? t('connect.pgbouncer_enabled_no_ipv4_text')
                        : sharedPoolerPreferred
                          ? t('connect.use_session_pooler_or_purchase_ipv4')
                          : t('connect.ipv4_addon_text'),
                    links: buttonLinks,
                  }}
                  parameters={[
                    { ...translatedConnectionParameters.host, value: connectionInfo.db_host },
                    { ...translatedConnectionParameters.port, value: connectionInfo.db_port },
                    { ...translatedConnectionParameters.database, value: connectionInfo.db_name },
                    { ...translatedConnectionParameters.user, value: connectionInfo.db_user },
                  ]}
              ...
              {selectedMethod === 'transaction' && IS_PLATFORM && (
                <ConnectionPanel
                  type="transaction"
                  title={translatedMethodOptions.transaction.label}
                  contentType={contentType}
                  lang={lang}
                  badge={poolerBadge}
                  fileTitle={fileTitle}
                  description={translatedMethodOptions.transaction.description}
                  connectionString={connectionStrings['pooler'][selectedTab]}
                  ipv4Status={{
                    type: !sharedPoolerPreferred && !ipv4Addon ? 'error' : 'success',
                    title:
                      !sharedPoolerPreferred && !ipv4Addon
                        ? t('connect.not_ipv4_compatible')
                        : t('connect.ipv4_compatible'),
                    description:
                      !sharedPoolerPreferred && !ipv4Addon
                        ? t('connect.pgbouncer_enabled_no_ipv4_text')
                        : sharedPoolerPreferred
                          ? t('connect.transaction_pooler_ipv4_proxied_free')
                          : t('connect.ipv4_addon_text'),
                    links: !sharedPoolerPreferred ? buttonLinks : undefined,
                  }}
                  notice={[t('connect.does_not_support_prepare_statements')]}
                  parameters={[
                    {
                      ...translatedConnectionParameters.host,
                      value: isReplicaSelected
                        ? connectionInfo.db_host
                        : poolingConfiguration?.db_host ?? '',
                    },
                    {
                      ...translatedConnectionParameters.port,
                      value: poolingConfiguration?.db_port.toString() ?? '6543',
                    },
                    {
                      ...translatedConnectionParameters.database,
                      value: poolingConfiguration?.db_name ?? '',
                    },
                    { ...translatedConnectionParameters.user, value: poolingConfiguration?.db_user ?? '' },
                    { ...translatedConnectionParameters.pool_mode, value: 'transaction' },
                  ]}
                        ...
                        >
                          ...
                          >
                            <div className="text-xs flex items-center gap-x-2 py-2 px-1">
                              <span>{t('connect.using_shared_pooler')}</span>
              ...
              {selectedMethod === 'session' && IS_PLATFORM && (
                <ConnectionPanel
                  type="session"
                  title={translatedMethodOptions.session.label}
                  contentType={contentType}
                  lang={lang}
                  badge="Shared Pooler"
                  fileTitle={fileTitle}
                  description={translatedMethodOptions.session.description}
                  ...
                  ipv4Status={{
                    type: 'success',
                    title: t('connect.ipv4_compatible'),
                    description: t('connect.session_pooler_ipv4_proxied_free'),
                    links: undefined,
                  }}
                  parameters={[
                    { ...translatedConnectionParameters.host, value: sharedPoolerConfig?.db_host ?? '' },
                    { ...translatedConnectionParameters.port, value: '5432' },
                    {
                      ...translatedConnectionParameters.database,
                      value: sharedPoolerConfig?.db_name ?? '',
                    },
                    { ...translatedConnectionParameters.user, value: sharedPoolerConfig?.db_user ?? '' },
                    { ...translatedConnectionParameters.pool_mode, value: 'session' },
                  ]}
          ...
          <Collapsible_Shadcn_ className="px-8 py-5">
            <CollapsibleTrigger_Shadcn_ className="group [&[data-state=open]>div>svg]:!-rotate-180">
              <div className="flex items-center gap-x-2 w-full">
                <p className="text-xs text-foreground-light group-hover:text-foreground transition">
                  {t('connect.connecting_to_sql_alchemy')}
                </p>
                <ChevronDown
            ...
            <CollapsibleContent_Shadcn_ className="my-2">
              <div className="text-foreground-light text-xs grid gap-2">
                <p>{t('connect.sql_alchemy_note')}</p>
                <p>{t('connect.sql_alchemy_example')}</p>
      ...
      <div className="px-8 pt-5 flex flex-col gap-y-1">
        <p className="text-sm">{t('connect.reset_password')}</p>
        <p className="text-sm text-foreground-lighter">
          <Trans
            i18nKey="connect.reset_password_note"
            components={{
              1: (
                <InlineLink
    ...
    <SelectItem_Shadcn_ value={method} className="[&>span:first-child]:top-3.5">
      <div className="flex flex-col w-full py-1">
        <div className="flex gap-x-2 items-center">
          {t(`connect.methods.${method}`)}
        </div>
        <div className="text-foreground-lighter text-xs">
          {t(`connect.methods.${method}_description`)}
        </div>
