      ...
      <div className="col-span-8 flex flex-col gap-2">
        {isTransactionDedicatedPooler && (
          <div className="text-xs flex items-center text-foreground-light">
            {t('connect.using_dedicated_pooler')}
          </div>
        )}
        <div className="flex flex-col -space-y-px">
          ...
          {type === 'transaction' && isSessionMode ? (
            <Admonition
              ...
              title={t('connect.transaction_pooler_unavailable')}
              description={t('connect.update_pool_mode_transaction')}
            >
        ...
        <div className="flex flex-col -space-y-px w-full">
          ...
          {type === 'session' && (
            <div className="border border-muted px-5 flex gap-7 items-center py-3 first:rounded-t last:rounded-b bg-alternative/50">
              ...
              <div className="flex flex-col">
                <span className="text-xs text-foreground">{t('connect.only_use_on_ipv4_network')}</span>
                <div className="flex flex-col text-xs text-foreground-lighter">
                    <p>{t('connect.session_pooler_ipv4_proxied_free')}</p>
                    <p>{t('connect.use_direct_connection_ipv6')}</p>
                  </div>
              </div>
            </div>
          )}
          ...
          {IS_PLATFORM && ipv4Status.type === 'error' && (
            <Collapsible_Shadcn_ className="group -space-y-px">
              ...
              >
                ...
                >
                  {t('connect.some_platforms_ipv4_only')}
                </Button>
              </CollapsibleTrigger_Shadcn_>
              <CollapsibleContent_Shadcn_ className="bg-dash-sidebar rounded-b border px-3 py-2">
                <div className="flex flex-col gap-2">
                  <p className="text-xs text-foreground-light max-w-xs">
                    {t('connect.platforms_ipv4_only_description')}
                  </p>
                  <div className="flex gap-4">
                    <div className="text-foreground text-xs">{t('connect.platform_names.vercel')}</div>
                    <div className="text-foreground text-xs">{t('connect.platform_names.github_actions')}</div>
                    <div className="text-foreground text-xs">{t('connect.platform_names.render')}</div>
                    <div className="text-foreground text-xs">{t('connect.platform_names.retool')}</div>
                  </div>
                  <p className="text-xs text-foreground-lighter max-w-xs">
                    <Trans
                      i18nKey="connect.purchase_ipv4_support"
                      components={{
                        1: (
                          <Link
                  ...
                  <p className="text-xs text-foreground-lighter max-w-xs">
                    {t('connect.use_session_or_transaction_pooler')}
                  </p>
                </div>
              </CollapsibleContent_Shadcn_>
            </Collapsible_Shadcn_>
