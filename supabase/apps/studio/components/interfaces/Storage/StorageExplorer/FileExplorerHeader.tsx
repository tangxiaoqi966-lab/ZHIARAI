...
export const FileExplorerHeader = ({
  ...
  const VIEW_OPTIONS = [
    { key: STORAGE_VIEWS.COLUMNS, name: t('storage.as_columns') },
    { key: STORAGE_VIEWS.LIST, name: t('storage.as_list') },
  ]

  const SORT_BY_OPTIONS = [
    { key: STORAGE_SORT_BY.NAME, name: t('storage.name') },
    { key: STORAGE_SORT_BY.CREATED_AT, name: t('storage.time_created') },
    { key: STORAGE_SORT_BY.UPDATED_AT, name: t('storage.time_modified') },
    { key: STORAGE_SORT_BY.LAST_ACCESSED_AT, name: t('storage.time_last_accessed') },
  ]

  const SORT_ORDER_OPTIONS = [
    { key: STORAGE_SORT_BY_ORDER.ASC, name: t('storage.ascending') },
    { key: STORAGE_SORT_BY_ORDER.DESC, name: t('storage.descending') },
  ]
  ...
  const onSetPathByString = async (paths: string[]) => {
    if (paths.length === 0) {
      ...
      setLoading({ isLoading: true, message: t('storage.navigating_to', { path: pathString }) })
        ...
        <div className="flex items-center space-x-1 px-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-40 min-w-0">
              ...
              <DropdownMenuSeparator />
              <DropdownMenuSub>
                <DropdownMenuSubTrigger>{t('storage.sort_by')}</DropdownMenuSubTrigger>
              ...
              <DropdownMenuSub>
                <DropdownMenuSubTrigger>{t('storage.sort_order')}</DropdownMenuSubTrigger>
                <DropdownMenuSubContent>
            ...
            tooltip={{
              content: {
                ...
                text: !canUpdateStorage
                  ? t('storage.upload_permission_error')
                  : undefined,
              },
            }}
          >
            {t('storage.upload_files')}
          </ButtonTooltip>
          <ButtonTooltip
            ...
            tooltip={{
              content: {
                side: 'bottom',
                text: !canUpdateStorage
                  ? t('storage.create_folder_permission_error')
                  : undefined,
              },
            }}
          >
            {t('storage.create_folder')}
          </ButtonTooltip>
        </div>
        ...
        <div className="flex items-center px-2">
          {snap.isSearching ? (
            <Input
              ...
              placeholder={t('storage.search_placeholder_explorer')}
              type="text"
              value={itemSearchString}
              onChange={(event: any) => setItemSearchString(event.target.value)}
            />
          ) : (
            <Button
              icon={<Search />}
