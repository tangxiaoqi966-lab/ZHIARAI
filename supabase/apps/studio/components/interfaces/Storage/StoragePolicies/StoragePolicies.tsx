...
export const StoragePolicies = () => {
  ...
  const { mutate: deleteDatabasePolicy, isPending: isDeletingPolicy } =
    useDatabasePolicyDeleteMutation({
      onSuccess: async () => {
        await refetch()
        toast.success(t('storage.successfully_deleted_policy'))
        setSelectedPolicyToDelete(undefined)
      },
      onError: (error: any) => {
        toast.error(t('storage.failed_to_delete_policy', { error: error.message }))
      },
    })
  ...
  const onSavePolicySuccess = async () => {
    toast.success(t('storage.successfully_saved_policy'))
    await refetch()
    onCancelPolicyEdit()
  }
  ...
  */
      ...
      return await Promise.all(
        payloads.map(async (payload) => {
          try {
            ...
            toast.error(t('storage.error_adding_policy', { error: error.message }))
            return true
          }
        })
      )
  ...
  const onUpdatePolicy = async (payload: any) => {
    ...
    try {
      ...
      toast.error(t('storage.error_updating_policy', { error: error.message }))
      return true
    }
  }
  ...
  return (
    <>
        ...
        {isLoading ? (
          ...
          <div>
            ...
            <PageSection>
              <PageSectionMeta>
                <PageSectionSummary>
                  <PageSectionTitle>{t('storage.schema')}</PageSectionTitle>
                  <PageSectionDescription>
                    {t('storage.schema_description')}
                  </PageSectionDescription>
                </PageSectionSummary>
              </PageSectionMeta>
              <PageSectionContent>
                <div className="flex flex-col gap-y-4">
                  ...
                  <StoragePoliciesBucketRow
                    table="objects"
                    label={t('storage.other_policies_objects')}
                  ...
                  <StoragePoliciesBucketRow
                    table="buckets"
                    label={t('storage.policies_buckets')}
      ...
      <ConfirmationModal
        ...
        variant="destructive"
        title={t('storage.delete_policy')}
        description={t('storage.delete_policy_confirm', { name: selectedPolicyToDelete?.name })}
        confirmLabel={t('storage.delete')}
        confirmLabelLoading={t('storage.deleting')}
