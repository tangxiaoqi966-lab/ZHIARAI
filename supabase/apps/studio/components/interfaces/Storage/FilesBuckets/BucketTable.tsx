  ...
  return (
    <BucketTableHeader>
      <BucketTableRow>
        ...
        <BucketTableHead className={stickyClasses}>{t('storage.name')}</BucketTableHead>
        <BucketTableHead className={stickyClasses}>{t('storage.policies')}</BucketTableHead>
        <BucketTableHead className={stickyClasses}>{t('storage.file_size_limit')}</BucketTableHead>
        <BucketTableHead className={stickyClasses}>{t('storage.allowed_mime_types')}</BucketTableHead>
  ...
  return (
    <BucketTableRow className="[&>td]:hover:bg-inherit">
      <BucketTableCell colSpan={5}>
        <p className="text-sm text-foreground">{t('storage.no_results_found')}</p>
        <p className="text-sm text-foreground-lighter">
          {t('storage.search_no_results', { filterString })}
        </p>
      </BucketTableCell>
    </BucketTableRow>
  )
    ...
    >
      ...
      <BucketTableCell className="flex-1">
        <div className="flex items-center gap-2.5">
          ...
          {bucket.public && <Badge variant="warning">{t('storage.public')}</Badge>}
        </div>
      </BucketTableCell>
      ...
      <BucketTableCell>
        ...
        >
          {bucket.file_size_limit
            ...
            : t('storage.unset', { limit: formattedGlobalUploadLimit })}
        </p>
      </BucketTableCell>
      ...
      <BucketTableCell>
        ...
        >
          {bucket.allowed_mime_types ? bucket.allowed_mime_types.join(', ') : t('storage.any')}
        </p>
      </BucketTableCell>
      ...
      <BucketTableCell>
        ...
        <button tabIndex={-1} className="sr-only">
          {t('storage.go_to_bucket_details')}
        </button>
      </BucketTableCell>
    </BucketTableRow>
  )
