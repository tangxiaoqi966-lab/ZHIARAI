      ...
      {!selectedTable ? (
        <div className="flex items-center justify-center h-full">
          <div className="w-[400px]">
            <Admonition
              type="default"
              title={t('table_editor.table_not_found', { id })}
              description={t('table_editor.table_not_found_description')}
            >
              {!!tabId ? (
                <Button
                  type="default"
                ...
                <Button
                  ...
                  onClick={() => setLastVisitedTable(undefined)}
                >
                  <Link href={`/project/${projectRef}/editor`}>{t('table_editor.head_back')}</Link>
                </Button>
              )}
            </Admonition>
          </div>
            ...
            customHeader={
              (isViewSelected || isTableSelected) && selectedView === 'definition' ? (
                <div className="flex items-center space-x-2">
                  <p>
                    <Trans
                      i18nKey="table_editor.sql_definition"
                      values={{ name: selectedTable.name }}
                      components={{ 1: <code className="text-sm" /> }}
                    />
                  </p>
                  <p className="text-foreground-light text-sm">{t('table_editor.read_only')}</p>
                </div>
              ) : null
            }
          >
