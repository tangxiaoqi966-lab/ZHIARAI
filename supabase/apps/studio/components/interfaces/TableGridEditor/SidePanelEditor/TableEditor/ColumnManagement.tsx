      ...
      <div className="w-full space-y-4 table-editor-columns">
            ...
            {isNewRecord && (
              <>
                ...
                {hasImportContent ? (
                  ...
                  >
                    {t('table_editor.import_data_from_csv')}
                  </Button>
                )}
              </>
            )}
        ...
        {hasImportContent && (
          <p className="text-sm text-foreground-light my-2">
            {t('table_editor.import_data_summary', { rowCount: importContent?.rowCount?.toLocaleString(), columnCount: columns.length })}
          </p>
        )}
        ...
        {primaryKeyColumns.length === 0 && (
          <Alert_Shadcn_ variant="warning">
            <WarningIcon />
            <AlertTitle_Shadcn_>{t('table_editor.no_primary_keys_title')}</AlertTitle_Shadcn_>
            <AlertDescription_Shadcn_>
              {t('table_editor.no_primary_keys_description')}
            </AlertDescription_Shadcn_>
          </Alert_Shadcn_>
        )}
        ...
        {primaryKeyColumns.length > 1 && (
          <InformationBox
            block
            icon={<Key size={16} />}
            title={t('table_editor.composite_primary_key_title')}
            description={t('table_editor.composite_primary_key_description')}
          />
        )}
        ...
        <div className="space-y-2">
          ...
          <div className="flex w-full px-3">
            ...
            <div className="w-[25%] flex items-center space-x-2">
              <h5 className="text-xs text-foreground-lighter">{t('table_editor.column_name')}</h5>
              <Tooltip>
                <TooltipTrigger>
                  <HelpCircle size={15} strokeWidth={1.5} className="text-foreground-lighter" />
                </TooltipTrigger>
                <TooltipContent side="bottom" className="w-[300px]">
                  {t('table_editor.column_name_tooltip')}
                </TooltipContent>
              </Tooltip>
            </div>
            <div className="w-[25%]">
              <h5 className="text-xs text-foreground-lighter">{t('table_editor.column_type')}</h5>
            </div>
            <div className={`${isNewRecord ? 'w-[25%]' : 'w-[30%]'} flex items-center space-x-2`}>
              <h5 className="text-xs text-foreground-lighter">{t('table_editor.column_default_value')}</h5>
              <Tooltip>
                <TooltipTrigger>
                  <HelpCircle size={15} strokeWidth={1.5} className="text-foreground-lighter" />
                </TooltipTrigger>
                <TooltipContent side="bottom" className="w-[300px]">
                  {t('table_editor.column_default_value_tooltip')}
                </TooltipContent>
              </Tooltip>
            </div>
            <div className="w-[10%]">
              <h5 className="text-xs text-foreground-lighter">{t('table_editor.column_primary')}</h5>
            </div>
            {/* Empty space */}
