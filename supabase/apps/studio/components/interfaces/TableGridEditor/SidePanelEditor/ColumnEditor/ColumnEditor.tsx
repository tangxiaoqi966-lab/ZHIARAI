...
export const ColumnEditor = ({
  ...
  return (
    <SidePanel
      ...
      customFooter={
        <ActionBar
          backButtonLabel={t('table_editor.cancel')}
          applyButtonLabel={t('table_editor.save')}
    ...
    >
      <FormSection header={<FormSectionLabel className="lg:!col-span-4">{t('table_editor.general')}</FormSectionLabel>}>
        <FormSectionContent loading={false} className="lg:!col-span-8">
          <Input
            label={t('table_editor.column_name')}
            type="text"
            descriptionText={t('table_editor.column_name_tooltip')}
          ...
          />
          <Input
            label={t('table_editor.description')}
            labelOptional={t('table_editor.description_optional')}
            type="text"
              ...
              <div className="space-y-2">
                <Button asChild type="default" size="tiny" icon={<Plus strokeWidth={2} />}>
                  <Link href={`/project/${ref}/database/types`} target="_blank" rel="noreferrer">
                    {t('table_editor.create_enum_types')}
                  </Link>
                </Button>
                <Button
                  asChild
                ...
                >
                  <Link
                    ...
                    rel="noreferrer"
                  >
                    {t('table_editor.about_data_types')}
                  </Link>
                </Button>
              </div>
            }
          >
            {t('table_editor.data_type')}
          </FormSectionLabel>
        }
      >
        <FormSectionContent loading={false} className="lg:!col-span-8">
          <ColumnType
            ...
            error={errors.format}
            description={
              lockColumnType ? t('table_editor.column_type_locked') : ''
            }
            ...
            <div className="space-y-4">
              {columnFields.format.includes('int') && (
                <div className="w-full">
                  <Checkbox
                    label={t('table_editor.is_identity')}
                    description={t('table_editor.is_identity_description')}
              ...
              {!columnFields.isPrimaryKey && (
                <div className="w-full">
                  <Checkbox
                    label={t('table_editor.define_as_array')}
                    description={t('table_editor.define_as_array_description')}
      ...
      <FormSection
        header={<FormSectionLabel className="lg:!col-span-4">{t('table_editor.foreign_keys')}</FormSectionLabel>}
      >
        <FormSectionContent loading={false} className="lg:!col-span-8">
          <ColumnForeignKey
            column={columnFields}
      ...
      <SidePanel.Separator />
      <FormSection
        header={<FormSectionLabel className="lg:!col-span-4">{t('table_editor.constraints')}</FormSectionLabel>}
      >
        <FormSectionContent loading={false} className="lg:!col-span-8">
          <Toggle
            label={t('table_editor.is_primary_key')}
            descriptionText={t('table_editor.is_primary_key_description')}
            checked={columnFields?.isPrimaryKey ?? false}
            onChange={() => onUpdateField({ isPrimaryKey: !columnFields?.isPrimaryKey })}
          />
          <Toggle
            label={t('table_editor.allow_nullable')}
            descriptionText={t('table_editor.allow_nullable_description')}
            checked={columnFields.isNullable}
            onChange={() => onUpdateField({ isNullable: !columnFields.isNullable })}
          />
          <Toggle
            label={t('table_editor.is_unique')}
            descriptionText={t('table_editor.is_unique_description')}
            checked={columnFields.isUnique}
            onChange={() => onUpdateField({ isUnique: !columnFields.isUnique })}
          />
          <Input
            label={t('table_editor.check_constraint')}
            labelOptional={t('table_editor.description_optional')}
