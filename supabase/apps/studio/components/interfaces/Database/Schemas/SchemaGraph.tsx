    ...
    <>
          ...
          <>
            ...
            {!hasNoTables && (
              <div className="flex items-center gap-x-2">
                ...
                >
                  {t('schema_graph.copy_as_sql')}
                </ButtonTooltip>
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <ButtonTooltip
                      aria-label={t('schema_graph.download_schema')}
                      type="default"
                      loading={isDownloading}
                      className="px-1.5"
                      icon={<Download />}
                      tooltip={{ content: { side: 'bottom', text: t('schema_graph.download_current_view') } }}
                    />
                  </DropdownMenuTrigger>
                  <DropdownMenuContent className="w-32">
                    <DropdownMenuItem onClick={() => downloadImage('png')}>
                      {t('schema_graph.download_as_png')}
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => downloadImage('svg')}>
                      {t('schema_graph.download_as_svg')}
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
                <ButtonTooltip
                  ...
                  tooltip={{
                    content: {
                      side: 'bottom',
                      text: t('schema_graph.auto_layout'),
                    },
                  }}
                >
                  {t('schema_visualizer.auto_layout')}
                </ButtonTooltip>
              </div>
            )}
          </>
      ...
      {isLoadingTables && (
        <div className="w-full h-full flex items-center justify-center gap-x-2">
          ...
          <p className="text-sm text-foreground-light">{t('schema_visualizer.loading_tables')}</p>
        </div>
      )}
      {isErrorTables && (
        <div className="w-full h-full flex items-center justify-center px-20">
          <AlertError subject={t('schema_visualizer.failed_retrieve_tables')} error={errorTables} />
        </div>
      )}
      {isSuccessTables && (
        <>
          {hasNoTables ? (
            <div className="flex items-center justify-center w-full h-full">
              <Admonition
                type="default"
                className="max-w-md"
                title={t('schema_visualizer.empty.title')}
                description={
                  isSchemaLocked
                    ? t('schema_visualizer.empty.managed_schema', { schema: selectedSchema })
                    : !canUpdateTables
                      ? t('schema_visualizer.empty.no_permission')
                      : t('schema_visualizer.empty.no_tables', { schema: selectedSchema })
                }
              >
                {canAddTables && (
                  <Button asChild className="mt-2" type="default" icon={<Plus />}>
                    <Link href={`/project/${ref}/editor?create=table`}>
                      {t('table_editor_sidebar_menu.menu.new_table')}
                    </Link>
                  </Button>
                )}
              </Admonition>
