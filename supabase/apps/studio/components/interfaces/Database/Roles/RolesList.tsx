...
export const RolesList = () => {
  ...
  const {
    ...
    onSuccess: () => {
      toast.success(t('roles.successfully_deleted'))
  ...
  return (
    <>
            ...
            <TooltipTrigger>
              <div className="w-42">
                <SparkBar
                  ...
                  labelBottom={t('roles.active_connections')}
                  labelBottomClass="text-xs"
                />
              </div>
            </TooltipTrigger>
            <TooltipContent align="start" side="bottom" className="space-y-1">
              <p className="text-foreground-light pr-2">{t('roles.connections_by_roles')}</p>
          ...
          <ButtonTooltip
            ...
            tooltip={{
              content: {
                side: 'bottom',
                text: !canUpdateRoles
                  ? t('roles.add_role_permission_error')
                  : undefined,
              },
            }}
          >
      ...
      <div className="space-y-4">
        <div>
          <div className="bg-surface-100 border border-default px-card py-3 rounded-t flex items-center space-x-4">
            <p className="text-sm text-foreground-light">{t('roles.roles_managed_by_supabase')}</p>
            <Badge variant="success">{t('roles.protected')}</Badge>
          </div>
        ...
        <div>
          <div className="bg-surface-100 border border-default px-card py-3 rounded-t">
            <p className="text-sm text-foreground-light">{t('roles.other_database_roles')}</p>
          </div>
      ...
      <ConfirmationModal
        ...
        title={t('roles.confirm_delete_title', { name: roleToDelete?.name })}
        onConfirm={deleteRole}
      >
        <p className="text-sm">
          {t('roles.confirm_delete_description')}
        </p>
      </ConfirmationModal>
    </>
  )
