...
export const TableList = ({
  ...
  return (
    <div className="flex flex-col gap-y-4">
      <div className="flex flex-col lg:flex-row lg:items-center gap-2 flex-wrap">
          ...
          <Popover_Shadcn_>
            ...
            <PopoverContent_Shadcn_ className="p-0 w-56" side="bottom" align="center">
              <div className="px-3 pt-3 pb-2 flex flex-col gap-y-2">
                <p className="text-xs">{t('table_list.show_entity_types')}</p>
        ...
        <div className="flex flex-grow justify-between gap-2 items-center">
          <Input
            ...
            placeholder={t('table_list.search_tables_placeholder')}
            value={filterString}
              ...
              tooltip={{
                content: {
                  ...
                  text: !canUpdateTables
                    ? t('table_list.no_permission_create_tables')
                    : undefined,
                },
              }}
            >
              {t('table_list.new_table')}
            </ButtonTooltip>
          )}
        </div>
      </div>
      ...
      {isError && <AlertError error={error} subject={t('schema_visualizer.failed_retrieve_tables')} />}
              ...
              <TableBody>
                <>
                        ...
                        {visibleTypes.length === 0 ? (
                          <>
                            <p className="text-sm text-foreground">
                              {t('table_list.select_entity_type_filter')}
                            </p>
                            <p className="text-sm text-foreground-light">
                              {t('table_list.no_results_filter_applied')}
                            </p>
                          </>
                        ) : (
                          <>
                            <p className="text-sm text-foreground">{t('table_list.no_tables_or_views')}</p>
                            <p className="text-sm text-foreground-light">
                              {t('table_list.no_entities_found', {
                                types: visibleTypes.length === 5
                                  ? t('table_list.tables')
                  ...
                  {entities.length === 0 && filterString.length > 0 && (
                    <TableRow key={selectedSchema}>
                      <TableCell colSpan={7}>
                        <p className="text-sm text-foreground">{t('table_list.no_results_found')}</p>
                        <p className="text-sm text-foreground-light">
                          {t('table_list.search_no_results', { filterString })}
                        </p>
                      </TableCell>
                    </TableRow>
                  )}
                      ...
                      <TableRow key={x.id}>
                        ...
                        <TableCell className="hidden lg:table-cell ">
                          {x.comment !== null ? (
                            ...
                            <p className="text-border-stronger">{t('table_list.no_description')}</p>
                          )}
                        </TableCell>
                          ...
                          <div className="flex justify-end gap-2">
                            ...
                            >
                              <Link href={`/project/${ref}/database/tables/${x.id}`}>
                                {t('table_list.columns', { count: x.columns.length })}
                              </Link>
                            </Button>
                            ...
                            {!isSchemaLocked && (
                              <DropdownMenu>
                                ...
                                <DropdownMenuContent side="bottom" align="end" className="w-40">
                                  ...
                                  >
                                    <Eye size={12} />
                                    <p>{t('table_list.view_in_table_editor')}</p>
                                  </DropdownMenuItem>
                                    ...
                                    <>
                                      ...
                                      <DropdownMenuItemTooltip
                                        ...
                                        tooltip={{
                                          content: {
                                            side: 'left',
                                            text: t('table_list.no_permission_edit_table'),
                                          },
                                        }}
                                      >
                                        <Edit size={12} />
                                        <p>{t('table_list.edit_table')}</p>
                                      </DropdownMenuItemTooltip>
                                      <DropdownMenuItemTooltip
                                        key="duplicate-table"
                                        className="gap-x-2"
                                        ...
                                        tooltip={{
                                          content: {
                                            side: 'left',
                                            text: t('table_list.no_permission_duplicate_tables'),
                                          },
                                        }}
                                      >
                                        <Copy size={12} />
                                        <span>{t('table_list.duplicate_table')}</span>
                                      </DropdownMenuItemTooltip>
                                      <DropdownMenuSeparator />
                                      <DropdownMenuItemTooltip
                                        ...
                                        tooltip={{
                                          content: {
                                            side: 'left',
                                            text: t('table_list.no_permission_delete_tables'),
                                          },
                                        }}
                                      >
                                        <Trash size={12} />
                                        <p>{t('table_list.delete_table')}</p>
                                      </DropdownMenuItemTooltip>
                                    </>
                                  )}
                                </DropdownMenuContent>
