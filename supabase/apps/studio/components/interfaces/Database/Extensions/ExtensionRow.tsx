...
export const ExtensionRow = ({ extension }: ExtensionRowProps) => {
  ...
  const { mutate: disableExtension, isPending: isDisabling } = useDatabaseExtensionDisableMutation({
    onSuccess: () => {
      toast.success(t('extensions.is_off', { name: extension.name }))
  ...
  return (
    <>
          ...
          <div className="flex items-center gap-x-2">
            ...
            {extensionMeta?.deprecated && extensionMeta?.deprecated.length > 0 && (
              <ButtonTooltip
                ...
                tooltip={{
                  content: {
                    text: t('extensions.deprecated_tooltip', { versions: extensionMeta.deprecated.join(', ') }),
                  },
                }}
              >
                {t('extensions.deprecated')}
              </ButtonTooltip>
            )}
          </div>
        </TableCell>
            ...
            <div className="flex flex-col gap-1">
              ...
              {!isOn && (
                <span className="text-foreground-lighter text-xs">
                  {t('extensions.install_to_use', { product: extensionMeta.product })}
                </span>
              )}
            </div>
          ) : (
              ...
              <Tooltip>
                ...
                {disabled && (
                  <TooltipContent side="bottom">
                    {!canUpdateExtensions
                      ? t('extensions.toggle_permission_error')
                      : orioleDbCheck
                        ? t('extensions.oriole_db_error')
                        : null}
                  </TooltipContent>
                )}
              </Tooltip>
      ...
      <ConfirmationModal
        ...
        title={t('extensions.disable_modal_title')}
        confirmLabel={t('extensions.disable')}
        variant="destructive"
        confirmLabelLoading={t('extensions.disabling')}
        loading={isDisabling}
      ...
      >
        <div className="flex flex-col gap-y-3">
          <p className="text-sm text-foreground-light">
            {t('extensions.disable_modal_description', { name: extension.name })}
          </p>
