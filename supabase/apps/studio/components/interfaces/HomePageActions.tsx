  ...
  return (
    <div className="flex items-center justify-between w-full">
      <div className="flex items-center gap-2">
        ...
        <FilterPopover
          name={t('home.filter_status')}
          title={t('home.filter_by_status')}
          options={[
            { key: PROJECT_STATUS.ACTIVE_HEALTHY, label: t('home.status_active') },
            { key: PROJECT_STATUS.INACTIVE, label: t('home.status_paused') },
          ]}
        ...
        <SortDropdown
          options={[
            { label: t('home.sort_by_name'), value: 'name' },
            { label: t('home.sort_by_created'), value: 'created' },
          ]}
          value={sort}
      ...
      <div className="flex items-center gap-2">
        ...
        {projectCreationEnabled && !hideNewProject && (
          <Button asChild icon={<Plus />} type="primary" size="tiny">
            <Link href={`/new/${slug}`}>{t('home.new_project')}</Link>
          </Button>
        )}
      </div>
    </div>
