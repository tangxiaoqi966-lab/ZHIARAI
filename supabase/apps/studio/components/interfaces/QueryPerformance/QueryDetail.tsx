...
import { getQueryPerformanceColumns } from './QueryPerformance.constants'
...
export const QueryDetail = ({ selectedRow, onClickViewSuggestion, onClose }: QueryDetailProps) => {
  ...
  const report = getQueryPerformanceColumns(t)
    ...
    <QueryPanelContainer>
      <QueryPanelSection className="pt-2 border-b relative">
        <div className="flex items-center justify-between mb-4">
          <h4>{t('advisor.query_pattern')}</h4>
          {showExplainWithAiInQueryPerformance && (
            <AiAssistantDropdown
              label={t('advisor.explain_with_ai')}
        ...
        >
          ...
          {isLinterWarning && (
            ...
            >
              <Lightbulb />
              <AlertTitle_Shadcn_>{t('advisor.suggested_optimization')}</AlertTitle_Shadcn_>
              <AlertDescription_Shadcn_>
                {t('advisor.optimization_description')}
              </AlertDescription_Shadcn_>
              <AlertDescription_Shadcn_>
                <Button className="mt-3" onClick={() => onClickViewSuggestion()}>
                  {t('advisor.view_suggestion')}
                </Button>
              </AlertDescription_Shadcn_>
            </Alert_Shadcn_>
          )}
      ...
      <QueryPanelSection className="pb-3 pt-6">
        <h4 className="mb-2">{t('advisor.metadata')}</h4>
