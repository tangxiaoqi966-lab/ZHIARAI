...
export const QueryPerformanceGrid = ({
  ...
  const columns = getQueryPerformanceColumns(t).map((col: any) => {
        ...
        if (col.id === 'query') {
          return (
            <div className="w-full flex items-center gap-x-3 group">
              ...
              {onCurrentSelectQuery && (
                <ButtonTooltip
                  tooltip={{ content: { text: t('advisor.query_details') } }}
    ...
    return (
      <div className="relative flex flex-grow bg-alternative min-h-0">
        <div className="flex-1 min-w-0 p-6">
          <Admonition
            type="destructive"
            title={t('advisor.failed_load_data')}
            description={error}
          >
            {onRetry && (
              <div className="mt-4">
            ...
            noRowsFallback: isLoading ? (
              ...
              <div className="absolute top-20 px-6 flex flex-col items-center justify-center w-full gap-y-2">
                ...
                <div className="text-center">
                  <p className="text-foreground">{t('advisor.no_queries_detected')}</p>
                  <p className="text-foreground-light">
                    {t('advisor.no_queries_criteria')}
                  </p>
                </div>
              </div>
            ),
