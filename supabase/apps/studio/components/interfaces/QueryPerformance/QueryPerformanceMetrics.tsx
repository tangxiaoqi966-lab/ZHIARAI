...
export const QueryPerformanceMetrics = () => {
  ...
  const stats = useMemo(() => {
    const slowQueriesTitle = queryMetrics?.[0]?.slow_queries === 1 ? t('advisor.metrics.slow_queries') : t('advisor.metrics.slow_queries')
    ...
    return [
      ...
      {
        title: t('advisor.metrics.cache_hit_rate'),
        value: queryMetrics?.[0]?.cache_hit_rate || '0%',
        tooltip: t('advisor.metrics.cache_hit_rate_tooltip'),
      },
      {
        title: t('advisor.metrics.avg_rows_per_call'),
        value: queryMetrics?.[0]?.avg_rows_per_call || '0',
        tooltip: t('advisor.metrics.avg_rows_per_call_tooltip'),
      },
    ]
  }, [queryMetrics, setSearchParams, t])
                  ...
                  {(card.title === 'Slow Queries' || card.title === 'Slow Query') && (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          type="button"
                          aria-label={t('advisor.metrics.slow_queries_calc_label')}
                      ...
                      <TooltipContent side="top" align="start" className="max-w-xs text-xs">
                        {t('advisor.metrics.slow_queries_tooltip')}
                      </TooltipContent>
                    </Tooltip>
                  )}
                  {card.tooltip && (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <button
                          type="button"
                          aria-label={t('advisor.metrics.what_is_label', { title: card.title })}
                          className="inline-flex h-4 w-4 items-center justify-center rounded-full bg-surface-200 text-foreground-lighter transition-colors hover:bg-surface-300 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-1 focus-visible:ring-foreground-lighter"
                          onClick={(e) => {
                            e.stopPropagation()
                          }}
