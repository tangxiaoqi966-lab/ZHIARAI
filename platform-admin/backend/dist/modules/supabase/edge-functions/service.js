...
function rowToEdgeFunction(row) {
    return {
        id: row.id,
        name: row.name,
        slug: row.slug,
...
class EdgeFunctionsService {
    ...
    // 列出 Edge Functions
    async listEdgeFunctions(projectRef, params = {}) {
        try {
            const projectId = await this.getProjectId(projectRef);
            if (!projectId) {
                throw types_1.EDGE_FUNCTIONS_ERRORS.FUNCTION_NOT_FOUND;
            }
    ...
    // 获取单个 Edge Function
    async getEdgeFunction(projectRef, functionId) {
        try {
            const projectId = await this.getProjectId(projectRef);
            if (!projectId) {
                throw types_1.EDGE_FUNCTIONS_ERRORS.FUNCTION_NOT_FOUND;
            }
            const result = await pool.query('SELECT * FROM supabase.edge_functions WHERE project_id = $1 AND id = $2', [projectId, functionId]);
    ...
    async createEdgeFunction(projectRef, functionRequest, userId) {
        try {
            const projectId = await this.getProjectId(projectRef);
            if (!projectId) {
                throw types_1.EDGE_FUNCTIONS_ERRORS.FUNCTION_NOT_FOUND;
            ...
            // 检查是否已存在
            const existing = await pool.query('SELECT id FROM supabase.edge_functions WHERE project_id = $1 AND slug = $2', [projectId, functionRequest.slug || functionRequest.name]);
            if (existing.rows.length > 0) {
                throw types_1.EDGE_FUNCTIONS_ERRORS.FUNCTION_ALREADY_EXISTS;
            }
            // 简化实现，实际需要处理代码部署
            const functionId = (0, uuid_1.v4)();
            const now = new Date().toISOString();
            const slug = functionRequest.slug || functionRequest.name.toLowerCase().replace(/[^a-z0-9_-]/g, '-');
            const result = await pool.query(`INSERT INTO supabase.edge_functions (
    ...
    // 更新 Edge Function
    async updateEdgeFunction(projectRef, functionId, functionRequest, userId) {
        try {
            const projectId = await this.getProjectId(projectRef);
            if (!projectId) {
                throw types_1.EDGE_FUNCTIONS_ERRORS.FUNCTION_NOT_FOUND;
            ...
            const result = await pool.query(`UPDATE supabase.edge_functions 
    ...
    // 删除 Edge Function
    async deleteEdgeFunction(projectRef, functionId) {
        try {
            const projectId = await this.getProjectId(projectRef);
            if (!projectId) {
                throw types_1.EDGE_FUNCTIONS_ERRORS.FUNCTION_NOT_FOUND;
            ...
            // 检查是否存在
            const existing = await pool.query('SELECT id FROM supabase.edge_functions WHERE project_id = $1 AND id = $2', [projectId, functionId]);
            if (existing.rows.length === 0) {
                throw types_1.EDGE_FUNCTIONS_ERRORS.FUNCTION_NOT_FOUND;
            }
            // 删除
            await pool.query('DELETE FROM supabase.edge_functions WHERE project_id = $1 AND id = $2', [projectId, functionId]);
        }
        catch (error) {
    ...
    // 部署 Edge Function
    async deployEdgeFunction(projectRef, functionId, deployRequest, userId) {
        try {
            const projectId = await this.getProjectId(projectRef);
            if (!projectId) {
                throw types_1.EDGE_FUNCTIONS_ERRORS.FUNCTION_NOT_FOUND;
            ...
            // 简化实现，实际需要部署到边缘计算平台
            const now = new Date().toISOString();
            const updateFields = ['status = $1', 'deployed_at = $2', 'deployed_by = $3', 'updated_at = $4'];
            const updateValues = ['ACTIVE', now, userId, now];
            let paramIndex = 5;
    ...
    // 激活 Edge Function
    async activateEdgeFunction(projectRef, functionId, userId) {
        return this.updateEdgeFunction(projectRef, functionId, { status: 'ACTIVE' }, userId);
    }
    // 停用 Edge Function
    async deactivateEdgeFunction(projectRef, functionId, userId) {
        return this.updateEdgeFunction(projectRef, functionId, { status: 'INACTIVE' }, userId);
    }
    // 批量操作
            ...
            try {
                ...
                switch (operation.type) {
                    ...
                    case 'deploy':
                        ...
                        const deployedFunction = await this.deployEdgeFunction(projectRef, operation.function_id, operation.deploy || {}, 'system');
    ...
    // 获取统计信息
    async getEdgeFunctionStats(projectRef) {
        try {
            const projectId = await this.getProjectId(projectRef);
            if (!projectId) {
                throw types_1.EDGE_FUNCTIONS_ERRORS.FUNCTION_NOT_FOUND;
    ...
    // 获取调用日志
    async getInvocationLogs(projectRef, functionId, limit = 100, offset = 0) {
        try {
            const projectId = await this.getProjectId(projectRef);
            if (!projectId) {
                throw types_1.EDGE_FUNCTIONS_ERRORS.FUNCTION_NOT_FOUND;
