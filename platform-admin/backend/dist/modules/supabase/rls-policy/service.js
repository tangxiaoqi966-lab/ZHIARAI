...
exports.RLSPolicyService = void 0;
...
class RLSPolicyService {
    ...
    // 列出策略
    async listPolicies(projectRef, params = {}) {
        try {
            const projectId = await this.getProjectId(projectRef);
            if (!projectId) {
                throw types_1.RLS_ERRORS.POLICY_NOT_FOUND;
            }
    ...
    // 获取单个策略
    async getPolicy(projectRef, policyId) {
        try {
            const projectId = await this.getProjectId(projectRef);
            if (!projectId) {
                throw types_1.RLS_ERRORS.POLICY_NOT_FOUND;
            }
            const result = await pool.query('SELECT * FROM supabase.rls_policies WHERE project_id = $1 AND id = $2', [projectId, policyId]);
    ...
    // 创建策略
    async createPolicy(projectRef, policyRequest, userId) {
        try {
            const projectId = await this.getProjectId(projectRef);
            if (!projectId) {
                throw types_1.RLS_ERRORS.POLICY_NOT_FOUND;
            ...
            // 检查策略是否已存在
            const existing = await pool.query('SELECT id FROM supabase.rls_policies WHERE project_id = $1 AND schema_name = $2 AND table_name = $3 AND name = $4', [projectId, policyRequest.schema || 'public', policyRequest.table, policyRequest.name]);
            if (existing.rows.length > 0) {
                throw types_1.RLS_ERRORS.POLICY_ALREADY_EXISTS;
            }
            ...
            if (!policyRequest.definition || policyRequest.definition.trim() === '') {
                throw types_1.RLS_ERRORS.POLICY_SYNTAX_ERROR;
            }
            const result = await pool.query(`INSERT INTO supabase.rls_policies (
    ...
    // 更新策略
    async updatePolicy(projectRef, policyId, policyRequest, userId) {
        ...
        try {
            ...
            const result = await pool.query(`UPDATE supabase.rls_policies 
    ...
    // 删除策略
    async deletePolicy(projectRef, policyId) {
        try {
            const projectId = await this.getProjectId(projectRef);
            if (!projectId) {
                throw types_1.RLS_ERRORS.POLICY_NOT_FOUND;
            ...
            // 检查策略是否存在
            const existing = await pool.query('SELECT id FROM supabase.rls_policies WHERE project_id = $1 AND id = $2', [projectId, policyId]);
            if (existing.rows.length === 0) {
                throw types_1.RLS_ERRORS.POLICY_NOT_FOUND;
            }
            // 删除策略
            await pool.query('DELETE FROM supabase.rls_policies WHERE project_id = $1 AND id = $2', [projectId, policyId]);
        }
        catch (error) {
    ...
    // 启用策略
    async enablePolicy(projectRef, policyId, userId) {
        return this.updatePolicy(projectRef, policyId, { enabled: true }, userId);
    }
    // 禁用策略
    async disablePolicy(projectRef, policyId, userId) {
    ...
    // 获取策略统计信息
    async getPolicyStats(projectRef) {
        try {
            const projectId = await this.getProjectId(projectRef);
            if (!projectId) {
                throw types_1.RLS_ERRORS.POLICY_NOT_FOUND;
            ...
            // 获取总策略数
            const totalResult = await pool.query('SELECT COUNT(*) as total FROM supabase.rls_policies WHERE project_id = $1', [projectId]);
            // 获取启用/禁用策略数
            const enabledResult = await pool.query('SELECT COUNT(*) as count FROM supabase.rls_policies WHERE project_id = $1 AND is_enabled = true', [projectId]);
            const disabledResult = await pool.query('SELECT COUNT(*) as count FROM supabase.rls_policies WHERE project_id = $1 AND is_enabled = false', [projectId]);
            // 按命令分组
            const commandResult = await pool.query('SELECT action as command, COUNT(*) as count FROM supabase.rls_policies WHERE project_id = $1 GROUP BY action', [projectId]);
            // 按schema分组
            const schemaResult = await pool.query('SELECT schema_name as schema, COUNT(*) as count FROM supabase.rls_policies WHERE project_id = $1 GROUP BY schema_name', [projectId]);
            // 按table分组
            const tableResult = await pool.query('SELECT table_name as table, COUNT(*) as count FROM supabase.rls_policies WHERE project_id = $1 GROUP BY table_name', [projectId]);
            const policies_by_command = {};
            commandResult.rows.forEach(row => {
                policies_by_command[row.command] = parseInt(row.count);
            });
            const policies_by_schema = {};
            schemaResult.rows.forEach(row => {
                policies_by_schema[row.schema] = parseInt(row.count);
            });
            const policies_by_table = {};
...
exports.RLSPolicyService = RLSPolicyService;
