...
export class AuthService {
  ...
  // 列出所有用户 (管理员接口)
  async listUsers(projectRef: string, params: ListUsersParams = {}): Promise<ListUsersResponse> {
    try {
      ...
      const users: AuthUser[] = result.rows.map(row => ({
        ...
        role: row.role || USER_ROLES.authenticated,
        email: row.email,
  ...
  // 获取单个用户
  async getUserById(projectRef: string, userId: string): Promise<AuthUser> {
    try {
      const projectId = await this.getProjectId(projectRef)
      if (!projectId) {
        throw { code: 'PROJECT_NOT_FOUND', message: '项目不存在', status: 404 }
      ...
      const result = await pool.query(
        'SELECT * FROM supabase.auth_users WHERE id = $1 AND instance_id = $2 AND deleted_at IS NULL',
        [userId, projectId]
      )
  ...
  // 创建用户 (管理员接口)
  async createUser(projectRef: string, userData: CreateUserRequest, adminId: string, ipAddress?: string, userAgent?: string): Promise<AuthUser> {
    ...
    try {
      ...
      const existingUser = await client.query(
        'SELECT id FROM supabase.auth_users WHERE email = $1 AND instance_id = $2 AND deleted_at IS NULL',
        [userData.email, projectId]
      )
      ...
      if (userData.password) {
        encryptedPassword = await bcrypt.hash(userData.password, 10)
      }

      // 插入用户
      const result = await client.query(
        `INSERT INTO supabase.auth_users (
        ...
        [
          ...
          userData.role || USER_ROLES.authenticated,
          userData.email,
          encryptedPassword,
      ...
      // 记录审计日志
      await logAuditEvent(client, projectId, adminId, AUTH_ACTIONS.CREATE_USER, 'user', userId, null, {
  ...
  // 更新用户
  async updateUser(projectRef: string, userId: string, userData: UpdateUserRequest, adminId: string, ipAddress?: string, userAgent?: string): Promise<AuthUser> {
    const client = await pool.connect()
    try {
      await client.query('BEGIN')
      ...
      // 记录审计日志
      await logAuditEvent(client, projectId, adminId, AUTH_ACTIONS.UPDATE_USER, 'user', userId, oldUser, {
        email: row.email,
        role: row.role
      }, ipAddress, userAgent)
  ...
  // 删除用户 (软删除)
  async deleteUser(projectRef: string, userId: string, adminId: string, ipAddress?: string, userAgent?: string): Promise<void> {
    const client = await pool.connect()
    try {
      await client.query('BEGIN')
      ...
      // 记录审计日志
      await logAuditEvent(client, projectId, adminId, AUTH_ACTIONS.DELETE_USER, 'user', userId, oldUser, null, ipAddress, userAgent)
  ...
  // 用户登录
  async signIn(projectRef: string, email: string, password: string, ipAddress?: string, userAgent?: string): Promise<AuthResponse> {
    try {
      const projectId = await this.getProjectId(projectRef)
      if (!projectId) {
        return {
      ...
      // 查找用户
      const result = await pool.query(
        'SELECT * FROM supabase.auth_users WHERE email = $1 AND instance_id = $2 AND deleted_at IS NULL',
        [email, projectId]
      )
      ...
      // 更新最后登录时间
      await pool.query(
        'UPDATE supabase.auth_users SET last_sign_in_at = $1 WHERE id = $2',
      ...
      const session: Session = {
        ...
        user: {
          ...
          role: user.role || USER_ROLES.authenticated,
          email: user.email,
      ...
      // 记录审计日志
      await logAuditEvent(pool, projectId, user.id, AUTH_ACTIONS.SIGN_IN, 'user', user.id, null, null, ipAddress, userAgent)
  ...
  async verifyToken(token: string): Promise<AuthUser | null> {
    try {
      ...
      const result = await pool.query(
        'SELECT * FROM supabase.auth_users WHERE id = $1 AND deleted_at IS NULL',
        [decoded.sub]
      )
  ...
  // 重置密码
  async resetPassword(projectRef: string, email: string, newPassword: string): Promise<boolean> {
    const client = await pool.connect()
    try {
      await client.query('BEGIN')
      ...
      const result = await client.query(
        `UPDATE supabase.auth_users 
         ...
         RETURNING id`,
        [encryptedPassword, new Date().toISOString(), email, projectId]
      )
  ...
  // 获取项目认证配置
  async getAuthConfig(projectRef: string): Promise<any> {
    try {
      const projectId = await this.getProjectId(projectRef)
      if (!projectId) {
        throw { code: 'PROJECT_NOT_FOUND', message: '项目不存在', status: 404 }
