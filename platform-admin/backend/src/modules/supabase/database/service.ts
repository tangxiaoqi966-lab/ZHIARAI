...
// 获取项目数据库连接池
function getProjectPool(projectRef: string): Pool {
...
export class DatabaseService {
  ...
  // 获取表列表
  async listTables(projectRef: string, schema: string = 'public'): Promise<string[]> {
    const sql = `
      SELECT table_name 
    ...
    try {
      ...
      throw DATABASE_ERRORS.QUERY_FAILED
    }
  }
  ...
  async getTableSchema(
    ...
    try {
      ...
      if (tableInfoResult.count === 0) {
        throw DATABASE_ERRORS.TABLE_NOT_FOUND
      }
      ...
      const columns: ColumnDefinition[] = columnsResult.data.map((row: any) => ({
      ...
      const primary_key = primaryKeyResult.data.map((row: any) => row.column_name)
      ...
      throw error.code ? error : DATABASE_ERRORS.QUERY_FAILED
    }
  }
  ...
  async queryData(
    ...
    try {
      ...
      if (params.order && Object.keys(params.order).length > 0) {
        const orders = Object.entries(params.order).map(([column, direction]) => 
  ...
  async insertData(
    ...
    try {
      ...
      const paramIndices = values.map((row, rowIndex) => 
  ...
  // 获取数据库统计信息
  async getDatabaseStats(projectRef: string): Promise<DatabaseStats> {
    try {
      const pool = getProjectPool(projectRef)
