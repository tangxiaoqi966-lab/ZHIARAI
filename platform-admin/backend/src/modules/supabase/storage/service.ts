...
// 转换数据库行到 Bucket 对象
function rowToBucket(row: any): StorageBucket {
  return {
    id: row.id,
    name: row.name,
    owner: row.owner,
...
// 转换数据库行到文件对象
function rowToStorageObject(row: any): StorageObject {
  return {
    name: row.name,
...
export class StorageService {
  ...
  // 列出所有 Bucket
  async listBuckets(projectRef: string): Promise<StorageBucket[]> {
    try {
      const projectId = await this.getProjectId(projectRef)
      if (!projectId) {
        throw STORAGE_ERRORS.BUCKET_NOT_FOUND
      }

      const result = await pool.query(
        'SELECT * FROM supabase.storage_buckets WHERE project_id = $1 ORDER BY created_at DESC',
        [projectId]
      )
  ...
  // 获取单个 Bucket
  async getBucket(projectRef: string, bucketName: string): Promise<StorageBucket> {
    try {
      const projectId = await this.getProjectId(projectRef)
      if (!projectId) {
        throw STORAGE_ERRORS.BUCKET_NOT_FOUND
      ...
      const result = await pool.query(
        'SELECT * FROM supabase.storage_buckets WHERE project_id = $1 AND name = $2',
  ...
  async createBucket(
    projectRef: string,
    bucketRequest: CreateBucketRequest,
    owner: string
    ...
    try {
      ...
      const existing = await pool.query(
        'SELECT id FROM supabase.storage_buckets WHERE project_id = $1 AND name = $2',
        [projectId, bucketRequest.name]
      )

      if (existing.rows.length > 0) {
        throw STORAGE_ERRORS.BUCKET_ALREADY_EXISTS
      }
      ...
      const result = await pool.query(
        `INSERT INTO supabase.storage_buckets (
      ...
      // 创建 Bucket 目录
      const bucketPath = path.join(STORAGE_ROOT, projectRef, bucketRequest.name)
  ...
  async updateBucket(
    projectRef: string,
    bucketName: string,
    bucketRequest: UpdateBucketRequest,
    updatedBy: string
  ): Promise<StorageBucket> {
    void updatedBy
    try {
      ...
      const result = await pool.query(
        `UPDATE supabase.storage_buckets 
  ...
  async deleteBucket(
    ...
    try {
      ...
      // 删除 Bucket 目录
      const bucketPath = path.join(STORAGE_ROOT, projectRef, bucketName)
      if (fs.existsSync(bucketPath)) {
        fs.rmSync(bucketPath, { recursive: true })
      }

      // 删除数据库记录
      await pool.query(
        'DELETE FROM supabase.storage_buckets WHERE project_id = $1 AND name = $2',
        [projectId, bucketName]
      )
    } catch (error: any) {
  ...
  async listFiles(
    ...
    try {
      ...
      `
      const queryParams: any[] = [projectId, bucketName]
      let paramIndex = 3
      ...
      const files: StorageObject[] = result.rows.map(rowToStorageObject)
    ...
    try {
      ...
      if (bucket.file_size_limit) {
        ...
        if (fileSize > bucket.file_size_limit) {
          throw STORAGE_ERRORS.FILE_TOO_LARGE
        }
      }
      ...
      if (bucket.allowed_mime_types && uploadRequest.contentType) {
        if (!bucket.allowed_mime_types.includes(uploadRequest.contentType)) {
          throw STORAGE_ERRORS.INVALID_FILE_TYPE
        }
      }

      // 构建文件路径
      const filePath = path.join(STORAGE_ROOT, projectRef, uploadRequest.bucket, uploadRequest.path)
      ...
      const fileId = uuidv4()
      const now = new Date().toISOString()
      const fileName = path.basename(uploadRequest.path)

      const result = await pool.query(
        `INSERT INTO supabase.storage_objects (
      ...
      if (result.rows.length === 0) {
        throw STORAGE_ERRORS.FILE_ALREADY_EXISTS
      }
  ...
  async downloadFile(
    ...
    try {
      ...
      const result = await pool.query(
        ...
        [projectId, bucketName, path.basename(filePath)]
      )

      if (result.rows.length === 0) {
        throw STORAGE_ERRORS.FILE_NOT_FOUND
      }

      const fileObject = rowToStorageObject(result.rows[0])
      
      // 读取文件内容
      const fullPath = path.join(STORAGE_ROOT, projectRef, bucketName, filePath)
      ...
      // 更新最后访问时间
      await pool.query(
        'UPDATE supabase.storage_objects SET last_accessed_at = $1 WHERE id = $2',
        [new Date().toISOString(), fileObject.buckets.id]
      )
  ...
  async deleteFile(
    ...
    try {
      ...
      // 删除数据库记录
      await pool.query(
        `DELETE FROM supabase.storage_objects 
         WHERE id IN (
           SELECT so.id 
           FROM supabase.storage_objects so
           JOIN supabase.storage_buckets sb ON so.bucket_id = sb.id
  ...
  async createSignedURL(
    ...
    bucketName: string,
    filePath: string,
    signedRequest: SignedURLRequest = {}
  ...
  async batchOperation(
    projectRef: string,
    batchRequest: BatchStorageOperationRequest
  ): Promise<BatchStorageOperationResponse> {
    const results = []
  ...
  // 获取存储统计信息
  async getStorageStats(projectRef: string): Promise<StorageStats> {
    try {
      const projectId = await this.getProjectId(projectRef)
      if (!projectId) {
        throw STORAGE_ERRORS.BUCKET_NOT_FOUND
      ...
      const bucketsResult = await pool.query(
        'SELECT COUNT(*) as total_buckets FROM supabase.storage_buckets WHERE project_id = $1',
        [projectId]
      )
